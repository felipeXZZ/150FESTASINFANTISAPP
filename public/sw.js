...
self.addEventListener("fetch", (event) => {
  ...
  // Entrada, saída e APIs nunca vêm do cache.
  if (url.pathname.startsWith("/sair") || url.pathname.startsWith("/api")) return;

  if (request.mode === "navigate") {
    // Saiu da conta: apaga as telas guardadas para não mostrar dados de ninguém.
    if (url.pathname === "/entrar") event.waitUntil(caches.delete(VERSAO));
    event.respondWith(
      fetch(request)
        .then((resposta) => {
          // Só guarda tela de verdade; redirecionamento para a entrada não.
