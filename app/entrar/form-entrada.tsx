...
export function FormEntrada() {
  ...
  return (
    <form action={acao}>
      <label htmlFor="email" className="block font-titulo text-xl leading-tight">
        Digite o e-mail que você usou na compra
      </label>
      <p className="mt-1.5 text-sm text-tinta-suave">Sem senha: é só o e-mail e você já entra.</p>
      ...
      <input
        ...
        className="mt-4 h-14 w-full rounded-xl border border-linha bg-papel/60 px-4 text-base placeholder:text-tinta/35 focus:border-azul focus:outline-none"
      />

      <button
        type="submit"
        disabled={enviando}
        className="mt-3 flex h-14 w-full items-center justify-center gap-2 rounded-xl bg-azul text-base font-semibold text-white transition active:bg-azul-escuro disabled:opacity-70"
      >
...
function Aviso({ mensagem }: { mensagem: string }) {
  return (
    <div className="mt-5 flex items-center gap-3 rounded-xl border border-ouro/60 bg-ouro-claro p-4">
      <p className="flex-1 text-sm">{mensagem}</p>
      <a
        href={linkWhatsapp("Olá! Não consigo entrar no 150 Festas Infantis.")}
        target="_blank"
