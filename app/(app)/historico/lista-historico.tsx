...
export function ListaHistorico() {
  ...
  if (calculos.length === 0) {
    return (
      <section className="flex min-h-[60dvh] flex-col items-center justify-center text-center">
        <History className="mb-4 size-10 text-azul" aria-hidden="true" />
        <p className="max-w-xs text-lg">
          Nenhum cálculo salvo ainda. Faça seu primeiro orçamento e ele aparece aqui.
        </p>
        <Link
          href="/calculadora"
          className="mt-6 flex h-14 items-center justify-center rounded-xl bg-azul px-8 text-base font-semibold text-white active:bg-azul-escuro"
        >
          Abrir calculadora
        </Link>
      </section>
  ...
  return (
    <section>
          ...
          <li key={c.id}>
            <Link href={`/historico/${c.id}`} className="flex items-center gap-3 px-4 py-3 active:bg-papel">
              <div className="min-w-0 flex-1">
                ...
                <p className="text-sm text-tinta/60">{dataCurta(c.criado_em)}</p>
              </div>
              <div className="shrink-0 text-right">
                <p className="font-titulo text-lg leading-tight tabular-nums">{brl(c.preco_final)}</p>
                <p
                  className={`text-sm font-semibold tabular-nums ${
                    c.lucro > 0 ? "text-verde-escuro" : "text-tinta/60"
                  }`}
                >
                  Lucro {brl(c.lucro)}
                </p>
      ...
      <p className="mt-3 px-1 text-sm text-tinta/60">
