...
export const metadata = { title: "Admin — 150 Festas Infantis" };
  ...
  return (
    <div className="mx-auto max-w-2xl">
      <div className="mb-4 flex items-center justify-between gap-4">
        ...
        <Link
          href="/admin/novo"
          className="flex h-11 items-center gap-1.5 rounded-xl bg-azul px-4 text-sm font-semibold text-white"
        >
      ...
      {modulos.length === 0 ? (
        <p className="rounded-2xl border border-dashed border-linha bg-white px-5 py-10 text-center text-tinta/70">
                ...
                <Link href={`/admin/${m.id}`} className="flex items-center gap-3 px-3 py-2.5 active:bg-papel">
                  <div className="grid aspect-video w-20 shrink-0 place-items-center overflow-hidden rounded-lg bg-linha/60">
                    {capa ? (
                      ...
                      <ImageOff className="size-5 text-tinta/30" aria-hidden="true" />
                    )}
                  </div>
                  <div className="min-w-0 flex-1">
                    <p className={`truncate font-semibold ${m.ativo ? "" : "text-tinta/40 line-through"}`}>
                      {m.titulo}
                    </p>
                    <p className="text-sm text-tinta/60">
                      #{m.ordem ?? 0} · {m.plano_minimo === "completo" ? "Pacote Completo" : "Básico"}
