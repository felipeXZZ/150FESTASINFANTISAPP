...
/**
 * Webhook de compra da GGCheckout, oferta +150 Festas Infantis (CLAUDE.md, seção 11).
 *
...
async function liberarAcesso(
  ...
  // Quem já é básico e paga de novo (outro pagamento, não reenvio do mesmo)
  // está comprando o upgrade, mesmo que o valor não tenha sido reconhecido.
  const segundaCompra =
    existente?.plano === "basico" &&
    Boolean(pagamentoId && existente.pagamento_id && pagamentoId !== existente.pagamento_id);

  // Nunca rebaixa: quem já tem o completo continua com ele.
  const plano: Plano =
    existente?.plano === "completo" || segundaCompra ? "completo" : planoComprado;
...
function tipoDaCompra(ids: string[], valor: number | null): Tipo | null {
  ...
  if (valor !== null) {
    ...
    // Upgrade custa menos que o completo, então é testado antes. Sem valor de
