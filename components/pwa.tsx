...
const CHAVE_DISPENSADO = "fi:instalar-dispensado";
...
export function FaixaInstalar() {
  ...
  // Na calculadora a faixa cobriria o botão da Biblioteca.
      ...
      <div className="mx-auto max-w-md rounded-2xl border border-linha bg-white p-3 shadow-lg">
        {instrucoes ? (
          <div>
            ...
            <ol className="mt-1 space-y-2 text-sm">
              <li className="flex items-center gap-2">
                1. Toque em <Share className="size-5 text-azul" aria-label="Compartilhar" /> na barra do Safari
              </li>
              <li className="flex items-center gap-2">
                2. Escolha <SquarePlus className="size-5 text-azul" aria-hidden="true" /> “Adicionar à Tela de
                Início”
              </li>
            </ol>
          </div>
          ...
          <div className="flex items-center gap-2">
            <Download className="size-5 shrink-0 text-azul" aria-hidden="true" />
            <p className="flex-1 text-sm">Instale na tela inicial e use como aplicativo</p>
            <button
              type="button"
              onClick={instalar}
              className="h-11 shrink-0 rounded-xl bg-azul px-4 text-sm font-semibold text-white"
            >
              Instalar
            </button>
            <BotaoFechar onClick={dispensar} />
...
function BotaoFechar({ onClick }: { onClick: () => void }) {
  return (
    <button
      ...
      className="-mr-1 grid size-11 shrink-0 place-items-center rounded-full text-tinta/50 active:bg-linha"
    >
