...
const CLASSE_INPUT =
  "h-12 w-full rounded-xl border border-linha bg-white px-3 text-base tabular-nums placeholder:text-tinta/35 focus:border-azul focus:outline-none";
...
export function CampoDinheiro({ rotulo, valor, onChange, dica, rotuloOculto }: Base) {
  ...
  return (
    <div>
      ...
      />
      {dica && <p className="mt-1.5 text-sm text-tinta/60">{dica}</p>}
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <div className="relative">
        ...
        />
        {sufixo && (
          <span className="pointer-events-none absolute inset-y-0 right-3 flex items-center text-tinta/50">
            {sufixo}
          </span>
        )}
      </div>
