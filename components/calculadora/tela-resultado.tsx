    ...
    <section className="flex min-h-[calc(100dvh-10rem)] flex-col">
      {titulo && (
        <header className="mb-4">
          ...
          {subtitulo && <p className="text-sm text-tinta/60">{subtitulo}</p>}
        </header>
      )}

      {semLucro && (
        <p className="mb-4 flex items-start gap-2.5 rounded-xl bg-ouro px-4 py-3 font-semibold text-tinta">
          <AlertTriangle className="mt-0.5 size-5 shrink-0" aria-hidden="true" />
          Nesse preço você não tem lucro. Aumente a margem ou revise os custos.
        </p>
      )}

      <p className="text-sm font-semibold text-tinta/60">Preço final</p>
      <p
        className={`font-titulo leading-[0.95] tracking-tight break-words tabular-nums ${
          semLucro ? "text-tinta" : "text-azul"
        } ${
      ...
      <p className={`mt-3 text-lg font-semibold ${semLucro ? "text-tinta/70" : "text-verde-escuro"}`}>
...
function LinhaValor({ rotulo, valor, forte }: { rotulo: string; valor: number; forte?: boolean }) {
  return (
    <div className={`flex items-center justify-between gap-4 py-3 ${forte ? "font-semibold" : ""}`}>
      <dt className={forte ? "" : "text-tinta/70"}>{rotulo}</dt>
