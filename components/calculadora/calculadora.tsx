...
const CHAVE_NEGOCIO = "fi:negocio";
      ...
      <TelaResultado resultado={calculo.resultado}>
        {salvoId ? (
          <Link
            ...
            className="flex h-14 items-center justify-center gap-2 rounded-xl bg-verde-escuro font-semibold text-white"
          >
    ...
    <div>
      <div className="mb-5">
        <div className="mb-2 flex items-center justify-between text-sm">
          ...
          {origem && <span className="truncate pl-4 text-tinta/60">Cópia de {origem}</span>}
        </div>
        <div
      ...
      <p className="mt-1 mb-5 text-tinta/70">{texto}</p>
      ...
      <div className="sticky bottom-[calc(4rem+env(safe-area-inset-bottom))] -mx-4 mt-6 border-t border-linha bg-papel/95 px-4 pt-3 pb-3 backdrop-blur">
        <div className="mx-auto max-w-xl">
          ...
          {ultimo && !calculo.ok && (
            <p role="alert" className="mb-2 rounded-lg bg-ouro-claro px-3 py-2 text-sm font-semibold">
              {calculo.erro}
            </p>
          )}
          <div className="flex gap-2">
            ...
            <button
              ...
              onClick={() => setPasso(passo + 1)}
              className="h-14 flex-1 rounded-xl bg-azul text-base font-semibold text-white active:bg-azul-escuro disabled:opacity-50"
            >
...
function Parcial({ passo, dados }: { passo: number; dados: Dados }) {
  ...
  return (
    <p className="mb-2 flex items-baseline justify-between" aria-live="polite">
      <span className="text-sm text-tinta/70">{parcial[0]}</span>
...
function BotaoRemover({ onClick, rotulo }: { onClick: () => void; rotulo: string }) {
  return (
    <button
      ...
      onClick={onClick}
      aria-label={rotulo}
      className="grid size-11 shrink-0 place-items-center rounded-full text-tinta/50 active:bg-linha"
    >
...
function BotaoAdicionar({ onClick }: { onClick: () => void }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className="flex h-12 w-full items-center justify-center gap-2 rounded-xl border border-dashed border-azul/50 font-semibold text-azul active:bg-azul/5"
    >
...
function PassoReutilizavel({ dados, atualizar }: PropsPasso) {
  ...
  return (
    <div className="space-y-3">
        ...
        <fieldset key={i} className="rounded-2xl border border-linha bg-white p-3">
          ...
          {item.valor > 0 && (
            <p className="mt-2 text-sm text-tinta/70">
              {item.usos > 0 ? (
                <>
                  Entra <strong className="text-tinta">{brl(item.valor / item.usos)}</strong> nesta festa
                </>
              ) : (
      ...
      />
      <p className="rounded-xl bg-white px-4 py-3 text-sm text-tinta/70">
  ...
  return (
    <>
      <button
        ...
        className="h-14 rounded-xl bg-azul text-base font-semibold text-white active:bg-azul-escuro"
      >
        Salvar cálculo
      </button>
      ...
      <dialog
        ...
        className="m-0 mt-auto w-full max-w-none rounded-t-3xl bg-papel p-0 text-tinta backdrop:bg-marinho/60 sm:m-auto sm:max-w-md sm:rounded-3xl"
      >
        <form onSubmit={salvar} className="pb-seguro p-5">
          ...
          <input
            ...
            className="mt-4 h-14 w-full rounded-xl border border-linha bg-white px-4 text-base placeholder:text-tinta/35 focus:border-azul focus:outline-none"
          />
          {erro && (
            <p role="alert" className="mt-3 rounded-lg bg-ouro-claro px-3 py-2 text-sm font-semibold">
              {erro}
            </p>
          )}
          <button
            type="submit"
            className="mt-4 flex h-14 w-full items-center justify-center rounded-xl bg-azul text-base font-semibold text-white active:bg-azul-escuro"
          >
            Salvar
          </button>
        </form>
